...
# Timestamp: 2019-04-28 18:40:02 UTC
# 
...
RUN apt-get update -qq \
    ...
    && curl -fsSL --retry 5 https://surfer.nmr.mgh.harvard.edu/pub/dist/freesurfer/6.0.1/freesurfer-Linux-centos6_x86_64-stable-pub-v6.0.1.tar.gz \
...
RUN apt-get update && apt-get install -y --no-install-recommends software-properties-common \
	...
	&& apt-get update \
	&& apt-get install -y --no-install-recommends python3.7 qt5-default\
	&& curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py \
	&& python3.7 /get-pip.py && rm /get-pip.py \
	&& rm /usr/bin/python3 \
	&& ln -s /usr/bin/python3.7 /usr/bin/python3 \
	&& ln -s /usr/bin/python3.7 /usr/bin/python \
	&& pip install pytest pyqt5 pyopengl

#Simnibs Setup
ARG RELEASE="latest"
ARG PIP_ADDITIONAL=""
RUN pip install -f https://github.com/simnibs/simnibs/releases/${RELEASE} fmm3dpy requests simnibs ${PIP_ADDITIONAL}
RUN mkdir -p /simnibs \
...
# Run set-up script with optional arg then enter bash shell
ENTRYPOINT ["/bin/bash", "/setup_env", "${LICENSE}"]
